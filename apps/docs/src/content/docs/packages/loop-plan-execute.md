---
title: '@moxxy/loop-plan-execute'
description: Plan-then-execute loop with an optional user-approval gate.
---

`@moxxy/loop-plan-execute` is a two-phase loop:
...
```sh
pnpm add @moxxy/loop-plan-execute
```
...
Switch interactively with `/loop plan-execute`.
...
- `planExecuteLoop`, `planExecuteLoopPlugin`
- `PLAN_EXECUTE_LOOP_NAME` — `'plan-execute'`
...
- [Loop strategies guide](../guides/loop-strategies).
