...
/**
 * Wraps an EmbeddingProvider with an in-memory content-hash cache. Useful when
 * the same text gets re-embedded across calls (e.g., a memory body that doesn't
 * change between recall invocations).
 *
 * For persistence across processes, use the cache's `serialize`/`hydrate`
 * methods.
 */
